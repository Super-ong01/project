...
//////////////// pin setup /////////////////
long Count_pulsesR = 0;
long Count_pulsesL = 0;

#define r_dir1_pin 13
#define r_dir2_pin 17
//#define r_pwm_pin 1

#define l_dir1_pin 18
#define l_dir2_pin 19
//#define l_pwm_pin 5

#define l_encoder_a 32
#define l_encoder_b 26

#define r_encoder_a 27
#define r_encoder_b 33
                 ...
                 long inTick)
{
  ...
  if (dirs > 0.5)
  {
    analogWrite(l_dir1_pin, control_outL);
    analogWrite(l_dir2_pin, 0);
    //analogWrite(l_pwm_pin, control_outL);
  }
  else if (dirs < -0.5)
  {
    analogWrite(l_dir1_pin, 0);
    analogWrite(l_dir2_pin, control_outL);
    //analogWrite(l_pwm_pin, control_outL);
...
void computePIDR(double control_cmd, long inTick)
{
  ...
  if (dirs > 0.5)
  {
    analogWrite(r_dir1_pin,control_outR);
    analogWrite(r_dir2_pin, 0);
    
  }
  else if (dirs < -0.5)
  {
    analogWrite(r_dir1_pin, 0);
    analogWrite(r_dir2_pin, control_outR);
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(l_encoder_a), DC_Motor_EncoderR, RISING);
  attachInterrupt(digitalPinToInterrupt(r_encoder_a), DC_Motor_EncoderL, RISING);
  micro_ros_init_successful = false;
...
void DC_Motor_EncoderR() {
  int motorRight = digitalRead(r_encoder_b);
  if (motorRight > 0) {
    Count_pulsesR--;
  } else {
    Count_pulsesR++;
...
void DC_Motor_EncoderL() {
  int motorRight = digitalRead(l_encoder_b);
  if (motorRight > 0) {
    Count_pulsesL--;
  } else {
    Count_pulsesL++;
